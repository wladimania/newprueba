...
public class SessionsServices {
    ...
    public SessionInfoDTO obtenerInfoSesion(Integer idUsuario) {
        SesionsInfoInterface sessionInfo = sessionssRepository.findLatestSessionInfoByUserId(idUsuario);
        if (sessionInfo == null) {
            // Si no hay ningún registro en absoluto para ese usuario, puedes optar por lanzar una excepción o devolver un objeto vacío.
            return  new SessionInfoDTO(0, null, null);
        }else return new SessionInfoDTO(sessionInfo.getIntentoFallido(), sessionInfo.getFechaIngreso(), sessionInfo.getFechaCierre());
