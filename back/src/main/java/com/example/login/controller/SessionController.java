...
public class SessionController {
    ...
    @GetMapping("/obtenerInfoSesion")
    public ResponseEntity<?> obtenerInfoSesion(@RequestParam("idUsuario") Integer idUsuario) {
        try {
            SessionInfoDTO sessionInfo = sessionsServices.obtenerInfoSesion(idUsuario);
            return ResponseEntity.ok(sessionInfo);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error al recuperar la información de la sesión: " + e.getMessage());
        }
    }
